...
func TestGorillaMiddlewareWithConfig(t *testing.T) {
	// set up libhoney to catch events instead of send them
	mo := &transmission.MockSender{}
	client, err := libhoney.NewClient(libhoney.ClientConfig{
		APIKey:       "placeholder",
	...
	incomingConfig := config.HTTPIncomingConfig{}

	// build the gorilla mux router with Middleware
	router := mux.NewRouter()
	router.Use(MiddlewareWithConfig(incomingConfig))
	router.HandleFunc("/hello/{name}", func(_ http.ResponseWriter, _ *http.Request) {})
	router.Handle("/yo", testHandler{})
	...
	t.Run("function handler", func(t *testing.T) {
		// build a sample request to generate an event
		r, _ := http.NewRequest("GET", "/hello/pooh", nil)
		r.Header.Add("Referer", "https://www.honeycomb.io/")
		...
		assert.Equal(t, 1, len(evs), "one event is created with one request through the Middleware")
		fields := evs[0].Data
		// verify that referer is set in span
		referer, ok := fields["request.referrer"]
		assert.True(t, ok, "request.referrer field must exist on middleware generated event")
		assert.Equal(t, "https://www.honeycomb.io/", referer, "referer var populated")
	})
